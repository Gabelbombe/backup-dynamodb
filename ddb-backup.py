...
def get_table_name_matches(conn, table_name_wildcard, separator):
    ...
    for table_name in all_tables:
        if table_name_wildcard is "*":
            matching_tables.append(table_name)
        elif separator is None:
            if table_name.startswith(table_name_wildcard.split("*", 1)[0]):
                matching_tables.append(table_name)
        elif table_name.split(separator, 1)[0] is table_name_wildcard.split("*", 1)[0]:
            matching_tables.append(table_name)
...
def get_restore_table_matches(table_name_wildcard, separator):
    ...
    for dir_name in dir_list:
        ...
        elif dir_name.split(separator, 1)[0] is table_name_wildcard.split("*", 1)[0]:
            matching_tables.append(dir_name)
...
def change_prefix(source_table_name, source_wildcard, destination_wildcard, separator):
    ...
    if source_table_name.split(separator, 1)[0] is source_prefix:
    ...
    if not args.dataOnly:
        while True:
            ...
            try:
                ...
                if e.body["__type"] is "com.amazonaws.dynamodb.v20120810#ResourceNotFoundException":
                    table_exist = False
                    logging.info(table_name + " table deleted!")
                    break
                elif e.body["__type"] is "com.amazonaws.dynamodb.v20120810#LimitExceededException":
                    logging.info("Limit exceeded, retrying deletion of " + table_name + "..")
                    time.sleep(sleep_interval)
                elif e.body["__type"] is "com.amazon.coral.availability#ThrottlingException":
                    logging.info("Control plane limit exceeded, retrying deletion of " + table_name + "..")
                    time.sleep(sleep_interval)
                elif e.body["__type"] is "com.amazonaws.dynamodb.v20120810#ResourceInUseException":
...
def mkdir_p(path):
    try:
        ...
        if exc.errno is exc.errno.EEXIST and os.path.isdir(path):
            pass
        else:
            raise
...
def wait_for_active_table(conn, table_name, verb):
    while True:
        if conn.describe_table(table_name)["Table"]["TableStatus"] is not "ACTIVE":
...
def update_provisioned_throughput(conn, table_name, read_capacity, write_capacity, wait=True):
    ...
    while True:
        try:
            ...
            if e.body["__type"] is "com.amazonaws.dynamodb.v20120810#LimitExceededException":
...
def do_backup(conn, table_name, read_capacity):
    ...
    if read_capacity is not None and read_capacity is not original_read_capacity:
...
def do_restore(conn, sleep_interval, source_table, destination_table, write_capacity):
    ...
    if args.excludeDataRestore in destination_table:
        ...
        for data_file in data_file_list:
            ...
            while len(items) > 0:
                ...
                if len(put_requests) is MAX_BATCH_WRITE:
    ...
    if not args.dataOnly and not args.skipThroughputUpdate and not args.excludeDataRestore:
        ...
        if write_capacity is not original_write_capacity:
        ...
        if table_global_secondary_indexes is not None:
            ...
            for gsi in table_global_secondary_indexes:
                ...
                if original_gsi_write_capacity is not gsi["ProvisionedThroughput"]["WriteCapacityUnits"]:
...
if args.mode is "backup":
    if args.srcTable.find("*") is not -1:
